              ...
              <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl overflow-x-auto no-scrollbar max-w-full">
                  {['all', 'author', 'editor', 'user'].map(r => (
                    <button 
                      ...
                      className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-all whitespace-nowrap ${roleFilter === r ? 'bg-white dark:bg-gray-700 shadow-sm text-purple-500' : 'text-gray-500 hover:text-purple-500'}`}
                    >
                      {r.toUpperCase()}
                    </button>
                  ))}

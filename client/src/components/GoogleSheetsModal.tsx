...
const GoogleSheetsModal = ({ isOpen, onClose, onSyncComplete }: GoogleSheetsModalProps) => {
    ...
    const handleConnect = async () => {
        try {
            ...
            const msg = error.response?.data?.message || "Failed to connect. Check permissions.";
            showError(msg);
        } finally {
            setIsLoading(false);
        }
    ...
    const handlePush = async () => {
        if (!window.confirm("This will overwrite the Google Sheet content. Continue?")) return;
        try {
            setIsLoading(true);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-md shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                    {/* Status Section */}
                    <div className={`p-4 rounded-xl border ${isConnected ? "bg-green-50 border-green-200 dark:bg-green-900/10 dark:border-green-800" : "bg-gray-50 border-gray-200 dark:bg-gray-700/30 dark:border-gray-600"}`}>
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-[10px] text-gray-400">
                            Ensure the service account email has "Editor" access to this sheet.
                        </p>
                    </div>
